...
public class Configuration {
	...
	/**
	 * 解析缓存中所有未处理的语句节点。一旦添加完所有映射器后就建议调用此方法，因为它提供了快速失败的语句验证。
	 ...
	 * 获取 SQL 语句映射对象步骤2
	 * @param id
	 * @param validateIncompleteStatements 验证SQL语句的完整性
	 * @return
	 */
	public MappedStatement getMappedStatement(String id, boolean validateIncompleteStatements) {
		...
		// Configuration 中 使用 Map<String,MappedStatement> 来保存 SQL 语句映射
		...
		public V put(String key, V value) {
			if (containsKey(key)) {
				throw new IllegalArgumentException(name + " already contains value for " + key
														   + (conflictMessageProducer == null ? "" : conflictMessageProducer
						.apply(super.get(key), value)));
			}
			if (key.contains(".")) {
