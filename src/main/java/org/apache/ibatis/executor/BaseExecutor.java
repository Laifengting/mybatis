...
public abstract class BaseExecutor implements Executor {
	...
	public <E> List<E> query(MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, CacheKey key,
		...
		try {
			// 查询堆栈数量 +1
			queryStack++;
			// 结果处理器是不是为null,如果是null,先从一级缓存中读取结果集，赋给 list，如果有结果处理器。将list赋值为null
		...
		if (queryStack == 0) {
			...
			// issue #601
			deferredLoads.clear();
			// 判断配置文件中的 本地缓存作用域，如果 等于 STATEMENT
	...
	public <E> List<E> query(MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler) throws SQLException {
		...
		// 创建缓存 key（一级缓存，在 BaseExecutor 中）
	...
	private <E> List<E> queryFromDatabase(MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler,
		...
		List<E> list;
		// 先将查询的 key 放入一级缓存中，并且是一个正在执行的标志
		localCache.putObject(key, EXECUTION_PLACEHOLDER);
		try {
			// 调用具体执行器的实现类的 doQuery() 方法执行数据库的查询操作(未指定执行器的时候，默认是SimpleExecutor)
			list = doQuery(ms, parameter, rowBounds, resultHandler, boundSql);
		} finally {
			// 通过 key 先从缓存中移除先前放入的对象
			localCache.removeObject(key);
		}
		// 重新将查询的结果 放入到缓存中
		localCache.putObject(key, list);
		// 判断 SQL 语句的类型状态，是不是可以调用状态，
		if (ms.getStatementType() == StatementType.CALLABLE) {
			// 如果是可以调用状态，将 key 和 SQL 语句需要的参数也放入一级缓存
